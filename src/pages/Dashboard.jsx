...
export default function Dashboard() {
  ...
  const criticalOpen   = MOCK_FINDINGS.filter(f => f.severity === 'critical' && f.status !== 'fixed').length
  const pendingTasks   = MOCK_TASKS.filter(t => t.status !== 'done').length
  const overdueTasks   = MOCK_TASKS.filter(t => t.status !== 'done' && t.due_date && t.due_date < today)

  const avgProgress = Math.round(MOCK_PROJECTS.reduce((s, p) => s + p.progress, 0) / MOCK_PROJECTS.length)

  const recentFindings = [...MOCK_FINDINGS].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 5)
  const urgentTasks = MOCK_TASKS.filter(t => t.status !== 'done').sort((a, b) => {
    const pr = { high: 0, medium: 1, low: 2 }
    return (pr[a.priority] ?? 2) - (pr[b.priority] ?? 2)
  }).slice(0, 4)

  const lastLog = MOCK_WORKLOG[0]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <Stat icon={FolderOpen}  label="פרויקטים פעילים" value={activeProjects}   sub={`מתוך ${MOCK_PROJECTS.length} סה"כ`}        color="text-blue-400" />
              ...
              {MOCK_PROJECTS.map(p => (
                <div key={p.id}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-green-400 font-bold">{p.progress}%</span>
                  </div>
          ...
          {lastLog && (
            <div className="bg-[#13161f] border border-[#1e2130] rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-white text-sm font-semibold">יומן — אתמול</span>
              </div>
