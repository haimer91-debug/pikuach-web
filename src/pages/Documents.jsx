    ...
    <div className="flex items-center gap-4 px-4 py-3 border-b border-[#1e2130]/50 hover:bg-[#1a1d27] transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-500 mt-0.5 flex items-center gap-2">
          <span>גרסה {doc.version}{Number(doc.revision) > 0 ? `.${doc.revision}` : ''}</span>
          <span>·</span>
          <span>{doc.size}</span>
          <span>·</span>
      ...
      <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <button className="w-7 h-7 bg-[#252840] rounded-lg flex items-center justify-center text-slate-400 hover:text-white" title="תצוגה מקדימה">
          <Eye size={13} />
        </button>
        <button className="w-7 h-7 bg-[#252840] rounded-lg flex items-center justify-center text-slate-400 hover:text-white" title="הורד">
          <Download size={13} />
        </button>
      </div>
    </div>
...
function ProfessionGroup({ prof, docs, defaultOpen = false }) {
  ...
  return (
    <div className="bg-[#13161f] border border-[#1e2130] rounded-xl overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#1a1d27] transition-colors">
      ...
      {open && (
        <div className="border-t border-[#1e2130]">
          {docs.map(doc => <FileRow key={doc.id} doc={doc} prof={prof} />)}
        </div>
      )}
    </div>
  )
      ...
      <div className="bg-[#13161f] border border-[#1e2130] rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" value={revision} onChange={e => setRevision(e.target.value)} min="0" className={inp}/>
            </div>
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-2 block">קובץ</label>
            <input ref={fileRef} type="file" accept=".pdf,.dwg,.xlsx,.docx,.xls,.doc" onChange={pickFile} className="hidden"/>
            {file ? (
              <div className="bg-[#1e2130] rounded-xl p-4 flex items-center gap-3">
                <File size={24} className="text-green-400 shrink-0"/>
                <div className="flex-1 min-w-0">
                  <div className="text-white text-sm font-medium truncate">{file.name}</div>
                  <div className="text-slate-500 text-xs">{fmt(file.size)}</div>
                </div>
                <button onClick={() => setFile(null)} className="text-slate-600 hover:text-red-400"><X size={14}/></button>
              </div>
            ) : (
              <button onClick={() => fileRef.current.click()}
                className="w-full border-2 border-dashed border-[#374151] rounded-xl p-8 text-center hover:border-green-500/50 transition-colors cursor-pointer block">
                <Upload size={28} className="text-slate-500 mx-auto mb-2"/>
                <p className="text-slate-400 text-sm">לחץ לבחירת קובץ</p>
                <p className="text-slate-600 text-xs mt-1">PDF, DWG, XLSX, DOCX</p>
              </button>
            )}
          </div>
        </div>
        <div className="flex gap-3 mt-5">
          <button onClick={save} disabled={!file}
            className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-[#1e2130] disabled:text-slate-500 disabled:cursor-not-allowed text-white text-sm py-2.5 rounded-lg font-medium transition-colors">
            {file ? 'הוסף מסמך' : 'בחר קובץ תחילה'}
          </button>
...
export default function Documents() {
  const [docs, setDocs] = useLocalStorage('pikuach_docs', INIT_DOCS)
  const [search, setSearch] = useState('')
  const [profFilter, setProfFilter] = useState(null)
  const [view, setView] = useState('grouped')
  const [showUpload, setShowUpload] = useState(false)

  function addDoc(doc) { setDocs(prev => [doc, ...prev]) }

  const filtered = docs.filter(d => {
    const matchSearch = !search || d.name.includes(search)
    const matchProf   = !profFilter || d.profession === profFilter
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white text-sm px-4 py-2 rounded-lg transition-colors">
            <Plus size={16}/> העלה מסמך
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 bg-[#13161f] border border-[#1e2130] rounded-lg px-3 py-2 mb-4">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש שם מסמך..."
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {PROFESSIONS.map(prof => {
          ...
          const active = profFilter === prof.id
          return (
            <button key={prof.id}
              onClick={() => setProfFilter(prev => prev === prof.id ? null : prof.id)}
              className={`text-xs px-3 py-1.5 rounded-full border transition-all hover:scale-105 ${prof.bg} ${prof.color} ${active ? 'ring-2 ring-offset-1 ring-offset-[#0f1117]' : 'opacity-80 hover:opacity-100'}`}
              style={active ? { ringColor: prof.color } : {}}>
              {prof.label} ({count})
            </button>
          )
        })}
        {(search || profFilter) && (
          <button onClick={() => { setSearch(''); setProfFilter(null) }}
            className="text-xs px-3 py-1.5 rounded-full border border-[#374151] text-slate-400 hover:text-white transition-colors">
            × נקה הכל
          </button>
        )}
      </div>
      ...
      {view === 'grouped' ? (
        <div className="space-y-3">
          {PROFESSIONS.map((prof, i) => (
            <ProfessionGroup key={prof.id} prof={prof} docs={filtered.filter(d => d.profession === prof.id)} defaultOpen={i < 3}/>
          ))}
      ...
      {showUpload && <UploadModal onClose={() => setShowUpload(false)} onSave={addDoc}/>}
    </div>
  )
}
